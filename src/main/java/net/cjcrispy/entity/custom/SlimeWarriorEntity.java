...
public class SlimeWarriorEntity extends HostileEntity implements GeoEntity {
	private final AnimatableInstanceCache geoCache = GeckoLibUtil.createInstanceCache(this);
	private int attackAnimationTimer = 0;
	private java.util.UUID ownerUuid = null;

	public SlimeWarriorEntity(EntityType<? extends HostileEntity> entityType, World world) {
	...
	public static DefaultAttributeContainer.Builder createAttributes() {
		return MobEntity.createMobAttributes()
			.add(EntityAttributes.GENERIC_MAX_HEALTH, MobConfig.SlimeMinion.MAX_HEALTH * 1.2) // 20% more health
			.add(EntityAttributes.GENERIC_MOVEMENT_SPEED, MobConfig.SlimeMinion.MOVEMENT_SPEED)
			.add(EntityAttributes.GENERIC_ATTACK_DAMAGE, MobConfig.SlimeMinion.ATTACK_DAMAGE * 1.5) // 50% more damage
